...
class LogConfigurator:
    ...
    def setup_logging(
        ...
        path = None
        # If a config file was set inside
        # the Environment Variable LOG_CFG, that has precedence
...
class log_configurator:
    ...
    def __init__(self):
        warnings.warn("Please use LogConfigurator class instead",
                      DeprecationWarning)
    ...
    def setup_logging(
        ...
        If the LOG_CFG env variable is set to point to a
        config file in yml, that is used to configure the logging.
        If the LOG_CFG is not set, but a config file is passed
        directly in yml format, the config will be applied.
        If no file is provided, it tries to read
        a default config file in
        <PROJECT_FOLDER>/config/default_logging_conf.yml
        If the default logging config is not found,
        a default config is set, where the level is WARNING and no
        formatting is applied.
        :param default_path:
        ...
        # If a config file was set inside
        # the Environment Variable LOG_CFG,
        # that has precedence
        value = os.getenv(env_key, None)
        if value and os.path.exists(value):
            path = value
        ...
        if path:
            logging.debug("Log configuration file found: " +
                          path)
            with open(path, 'rt') as f:
                config = yaml.safe_load(f.read())
            logging.config.dictConfig(config)
